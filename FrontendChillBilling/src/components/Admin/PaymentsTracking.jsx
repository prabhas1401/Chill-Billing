...
export default function PaymentsTracking() {
  ...
  // Calculate last 30 days successful payments
  ...
  useEffect(() => {
    ...
    if (search) {
      data = data.filter((p) => {
        const invoiceNumber = p.invoiceNumber || "";  // Default to empty string if null or undefined
        const transactionId = p.transactionId || "";  // Default to empty string if null or undefined
        return (
  ...
  // Format payment date to only show the date (no time)
