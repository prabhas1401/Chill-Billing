...
export default function AddInvoice() {
  ...
  useEffect(() => {
    const fetchProductsAndCustomers = async () => {
      try {
        ...
        // Map customers to include email and name
        const customerOptions = customersRes.data.map((customer) => ({
          email: customer.emailId,
          name: customer.fullName,
        }));
        setCustomers(customerOptions);
        setFilteredCustomers(customerOptions); // Initial display of all customers
      } catch (err) {
  ...
  // Handle selection of a customer from the list
  ...
  const handleProductChange = (index, field, value) => {
    ...
    // Set quantity to 1 if product selected and quantity is 0
  ...
  const updateQuantity = (index, change) => {
    ...
    // Only allow quantity change if product selected
      ...
      <div className="add-invoice-container">
        ...
        <form onSubmit={handleSubmit} className="invoice-form">
          ...
          <input
            ...
            list="customer-list" // Datalist to bind to the input
          />
